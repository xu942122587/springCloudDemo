...
public class MessageContent {
	...
	// 换卡成功
	private final static String USER_BANK_CHANGE_PASS_CONTENT = "尊敬的用户，您的银行卡换卡申请已经通过，银行卡已经替换。详情请登录APP查看。";

	// 还款准到账短信
	private final static String ACCOUNT_REPAYMENT_CONTENT = "尊敬的用户，您明日将会有{}笔本金共计{}元，{}笔利息共计{}元到账，详情请登录查看。";
	...
	public static String getUserBankChangePassContent() {
	    return USER_BANK_CHANGE_PASS_CONTENT;
    }

    public static String getAccountRepaymentContent(RepaymentMSNDto dto) throws Exception {
		return StringUtils.format(WITHDRAW_APPLY_CONTENT,
				dto.getCapitalCount(), AmountUtils.changeF2Y(dto.getCapitalAmount()),
				dto.getInterestCount(), AmountUtils.changeF2Y(dto.getInterestAmount()));
	}
